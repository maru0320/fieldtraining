...
const ClassUpdate = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axiosInstance.get(`/class/detail?boardID=${boardID}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("/class/update", formData, {
        headers: {
  ...
  const handleFileDelete = async (fileID) => {
    try {
      await axiosInstance.get(`/class/update/fileDelete?fileID=${fileID}`);
      alert("파일이 삭제되었습니다.");
