...
const UnionBoardDetail = () => {
  ...
  const deleteBoard = async () => {
    try {
      await axiosInstance.get(
        `/union/detail/delete?boardID=${boardID}`
      );
      alert("게시글이 삭제되었습니다!");
      navigate("/Union");
    } catch (error) {
  ...
  const createComment = async () => {
    try {
      await axiosInstance.post("/union/detail/commentWrite", {
        boardID: boardID,
  ...
  const deleteComment = async (commentID) => {
    try {
      await axiosInstance.get(
        `/union/detail/commentDelete?commentID=${commentID}`
      );
      alert("댓글이 삭제되었습니다!");
      fetchBoardDetail();
    } catch (error) {
