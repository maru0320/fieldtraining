...
import "./ReferenceBoardDetail.css";
import axiosInstance from "../../axiosInstance";
import { FaList, FaEdit, FaTrash } from "react-icons/fa";

const ReferenceBoardDetail = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axiosInstance.get(
        `/reference/detail?boardID=${boardID}`
      );
  ...
  const deleteBoard = async () => {
    try {
      await axiosInstance.get(
        `/reference/detail/delete?boardID=${boardID}`
      );
      alert("게시글이 삭제되었습니다!");
      navigate("/Reference");
    } catch (error) {
  ...
  const handleUpdateClick = () => {
    navigate(`/Reference/ReferenceUpdate/${boardID}`);
  };
  ...
  const createComment = async () => {
    try {
      await axiosInstance.post("/reference/detail/commentWrite", {
        boardID: boardID,
  ...
  const deleteComment = async (commentID) => {
    try {
      await axiosInstance.get(
        `/reference/detail/commentDelete?commentID=${commentID}`
      );
      alert("댓글이 삭제되었습니다!");
      fetchBoardDetail();
    } catch (error) {
                      ...
                      <div className="board-files">
                        {boardDetail.files.map((file, index) => (
                          <p key={index}>
                            {file.originalName}{" "}
                            <a
                              href={`http://localhost:8090/reference/download?filePath=${encodeURIComponent(
                                file.storedPath
                              )}`}
                              download
                            >
...
export default ReferenceBoardDetail;
