...
const Rating = () => {
  ...
  const fetchBoardList = async () => {
    try {
      const response = await axiosInstance.get("/rating/list", {
        params: {
          userId: sub,
          page: currentPage,
          size: pageSize,
  ...
  const handleWriteClick = () => {
    navigate("/Rating/RatingWrite");
  };

  const handleDetailClick = (boardID) => {
    navigate(`/Rating/PractcieBoardDetail/${boardID}`);
  };
  ...
  return (
    <div className="Rating-body">
        <Sidebar />
      <form className="Ratingboard-container" onSubmit={handleSearch}>
        <h1 className="Ratingboard-title">평가</h1>
        {error && <p className="error-message">{error}</p>}
        <table className="Ratingboard-table">
          <thead>
            <tr>
              <th>번호</th>
              <th>제목</th>
        ...
        <div className="Ratingboard-footer">
