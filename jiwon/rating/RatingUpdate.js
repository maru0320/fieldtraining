...
const RatingUpdate = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axiosInstance.get(`/rating/detail?boardID=${boardID}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("/rating/update", formData, {
        headers: {
  ...
  const handleFileDelete = async (fileID) => {
    try {
      await axiosInstance.get(`/rating/update/fileDelete?fileID=${fileID}`);
      alert("파일이 삭제되었습니다.");
  ...
  return (
    <div className="Ratingwrite-container">
      <Sidebar/>
      <h1 className="Ratingwrite-title">게시글 수정</h1>
      <form onSubmit={handleSubmit} className="Ratingwrite-form">
        <input
          type="text"
          placeholder="제목을 입력하세요"
          value={boardDetail.title || ""}
          onChange={(e) => setBoardDetail({ ...boardDetail, title: e.target.value })}
          className="Ratingwrite-input"
        />
        <textarea
          placeholder="내용을 입력하세요"
          value={boardDetail.content || ""}
          onChange={(e) => setBoardDetail({ ...boardDetail, content: e.target.value })}
          className="Ratingwrite-textarea"
        />
        {/* 기존 파일 리스트 */}
        <div>
          <h3>첨부된 파일</h3>
