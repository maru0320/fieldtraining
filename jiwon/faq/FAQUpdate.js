...
const FAQUpdate = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axiosInstance.get(`/faq/detail?boardID=${boardID}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("/faq/update", formData, {
        headers: {
  ...
  const handleFileDelete = async (fileID) => {
    try {
      await axiosInstance.get(`/faq/update/fileDelete?fileID=${fileID}`);
      alert("파일이 삭제되었습니다.");
