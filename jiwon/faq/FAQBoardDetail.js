...
const FAQBoardDetail = () => {
  ...
  const deleteBoard = async () => {
    try {
      await axiosInstance.get(
        `/faq/detail/delete?boardID=${boardID}`
      );
      alert("게시글이 삭제되었습니다!");
      navigate("/FAQ");
    } catch (error) {
  ...
  const handleUpdateClick = () => {
    navigate(`/FAQ/FAQUpdate/${boardID}`);
  };
  ...
  const createComment = async () => {
    try {
      await axiosInstance.post("/faq/detail/commentWrite", {
        boardID: boardID,
  ...
  const deleteComment = async (commentID) => {
    try {
      await axiosInstance.get(
        `/faq/detail/commentDelete?commentID=${commentID}`
      );
      alert("댓글이 삭제되었습니다!");
      fetchBoardDetail();
    } catch (error) {
                  ...
                  <div className="board-files">
                    {boardDetail.files.map((file, index) => (
                      <p key={index}>
                        {file.originalName}{" "}
                        <a
                          href={`http://localhost:8090/faq/download?filePath=${encodeURIComponent(
                            file.storedPath
                          )}`}
                          download
                        >
