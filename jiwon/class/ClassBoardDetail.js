...
const ClassBoardDetail = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:8090/class/detail?boardID=${boardID}`
      );
  ...
  const deleteBoard = async () => {
    try {
      await axios.get(
        `http://localhost:8090/class/detail/delete?boardID=${boardID}`
      );
      alert("게시글이 삭제되었습니다!");
      navigate("/Class");
    } catch (error) {
  ...
  const createComment = async () => {
    try {
      await axios.post("http://localhost:8090/class/detail/commentWrite", {
        boardID: boardID,
  ...
  const deleteComment = async (commentID) => {
    try {
      await axios.get(
        `http://localhost:8090/class/detail/commentDelete?commentID=${commentID}`
      );
      alert("댓글이 삭제되었습니다!");
      fetchBoardDetail();
    } catch (error) {
