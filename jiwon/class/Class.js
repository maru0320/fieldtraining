...
const Class = () => {
  ...
  const fetchBoardList = async () => {
    try {
      const response = await axios.get("http://localhost:8090/class/list", {
        params: {
          userId: sub,
          page: currentPage,
          size: pageSize,
  ...
  const handleWriteClick = () => {
    navigate("/Class/ClassWrite");
  };

  const handleDetailClick = (boardID) => {
    navigate(`/Class/ClassBoardDetail/${boardID}`);
  };
  ...
  return (
    <div className="mypage-body">
      ...
      <form className="board-container" onSubmit={handleSearch}>
        <h1 className="board-title">수업일지</h1>
