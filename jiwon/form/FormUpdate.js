...
const FormUpdate = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axiosInstance.get(`/form/detail?boardID=${boardID}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("/form/update", formData, {
        headers: {
  ...
  const handleFileDelete = async (fileID) => {
    try {
      await axiosInstance.get(`/form/update/fileDelete?fileID=${fileID}`);
      alert("파일이 삭제되었습니다.");
