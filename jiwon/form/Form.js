...
const Form = () => {
  ...
  const fetchBoardList = async () => {
    try {
      const response = await axios.get("http://localhost:8090/form/list", {
        params: {
          page: currentPage,
          size: pageSize,
  ...
  const handleWriteClick = () => {
    navigate("/Form/FormWrite");
  };

  const handleDetailClick = (boardID) => {
    navigate(`/Form/FormBoardDetail/${boardID}`);
  };
  ...
  return (
    <div className="board-body">
      ...
      <form className="board-container" onSubmit={handleSearch}>
        <h1 className="board-title">양식게시판</h1>
