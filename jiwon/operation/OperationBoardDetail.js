...
import "./OperationBoardDetail.css";

const OperationBoardDetail = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:8090/operation/detail?boardID=${boardID}`
      );
  ...
  const deleteBoard = async () => {
    try {
      await axios.get(
        `http://localhost:8090/operation/detail/delete?boardID=${boardID}`
      );
      alert("게시글이 삭제되었습니다!");
      navigate("/Operation");
    } catch (error) {
  ...
  const handleUpdateClick = () => {
    navigate(`/Operation/OperationUpdate/${boardID}`);
  };
  ...
  const createComment = async () => {
    try {
      await axios.post("http://localhost:8090/operation/detail/commentWrite", {
        boardID: boardID,
  ...
  const deleteComment = async (commentID) => {
    try {
      await axios.get(
        `http://localhost:8090/operation/detail/commentDelete?commentID=${commentID}`
      );
      alert("댓글이 삭제되었습니다!");
      fetchBoardDetail();
    } catch (error) {
                  ...
                  <div className="board-files">
                    {boardDetail.files.map((file, index) => (
                      <p key={index}>
                        {file.originalName}{" "}
                        <a
                          href={`http://localhost:8090/operation/download?filePath=${encodeURIComponent(
                            file.storedPath
                          )}`}
                          download
                        >
...
export default OperationBoardDetail;
