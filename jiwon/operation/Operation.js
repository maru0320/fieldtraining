...
const Operation = () => {
  ...
  const fetchBoardList = async () => {
    try {
      const response = await axios.get("http://localhost:8090/operation/list", {
        params: {
          userId: sub,
          page: currentPage,
          size: pageSize,
  ...
  const handleWriteClick = () => {
    navigate("/Operation/OperationWrite");
  };

  const handleDetailClick = (boardID) => {
    navigate(`/Operation/OperationBoardDetail/${boardID}`);
  };
  ...
  return (
    <div className="mypage-body">
      ...
      <form className="board-container" onSubmit={handleSearch}>
        <h1 className="board-title">학급운영일지</h1>
