...
const OperationUpdate = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axios.get(`http://localhost:8090/operation/detail?boardID=${boardID}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:8090/operation/update", formData, {
        headers: {
  ...
  const handleFileDelete = async (fileID) => {
    try {
      await axios.get(`http://localhost:8090/operation/update/fileDelete?fileID=${fileID}`);
      alert("파일이 삭제되었습니다.");
