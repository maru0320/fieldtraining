...
import "./UnionBoardDetail.css";

const UnionBoardDetail = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:8090/union/detail?boardID=${boardID}`
      );
  ...
  const deleteBoard = async () => {
    try {
      await axios.get(
        `http://localhost:8090/union/detail/delete?boardID=${boardID}`
      );
      alert("게시글이 삭제되었습니다!");
      navigate("/Union");
    } catch (error) {
  ...
  const handleUpdateClick = () => {
    navigate(`/Union/UnionUpdate/${boardID}`);
  };
  ...
  const createComment = async () => {
    try {
      await axios.post("http://localhost:8090/union/detail/commentWrite", {
        boardID: boardID,
  ...
  const deleteComment = async (commentID) => {
    try {
      await axios.get(
        `http://localhost:8090/union/detail/commentDelete?commentID=${commentID}`
      );
      alert("댓글이 삭제되었습니다!");
      fetchBoardDetail();
    } catch (error) {
                  ...
                  <div className="board-files">
                    {boardDetail.files.map((file, index) => (
                      <p key={index}>
                        {file.originalName}{" "}
                        <a
                          href={`http://localhost:8090/union/download?filePath=${encodeURIComponent(
                            file.storedPath
                          )}`}
                          download
                        >
...
export default UnionBoardDetail;
