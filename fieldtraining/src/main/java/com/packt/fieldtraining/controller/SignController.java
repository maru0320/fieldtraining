package com.packt.fieldtraining.controller;
...
public class SignController {
    ...
    public ResponseEntity<SignInResultDto> signIn(@RequestBody SignInRequestDto signInRequestDto) {
        String id = signInRequestDto.getId();
        String password = signInRequestDto.getPassword();

        LOGGER.info("[login] 로그인을 시도하고 있습니다. id : {}", id);

        // 로그인 서비스 호출
        try {
            SignInResultDto signInResultDto = signService.login(id, password);

            if (signInResultDto.getCode() == 0) {
                LOGGER.info("[login] 정상적으로 로그인되었습니다. id: {}, token : {}", id, signInResultDto.getToken());
                return ResponseEntity.ok(signInResultDto);  // 로그인 성공 시 200 OK 반환
            } else {
                LOGGER.warn("[login] 로그인 실패 id: {}", id);
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(signInResultDto);  // 인증 실패 시 401 Unauthorized
            }
        } catch (RuntimeException e) {
            LOGGER.error("[login] 로그인 중 오류 발생 id: {}", id, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();  // 예외 처리 시 500 Internal Server Error 반환
        }
    }

    // 회원가입 엔드포인트
    @PostMapping("/generaljoin")  // 절대 경로를 상대 경로로 수정
    public ResponseEntity<SignUpResultDto> join(@RequestBody SignUpRequestDto signUpRequestDto) {
        ...
        // 회원가입 서비스 호출
        try {
            SignUpResultDto signUpResultDto = signService.join(
