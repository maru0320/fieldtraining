...
const validateForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setResetFormSignal(true); // 초기화 신호 활성화
                setTimeout(() => setResetFormSignal(false), 100); // 신호 비활성화
            } else {
    ...
    function partChoice() {
        ...
        if (value === 'student') {
            return <StudentJoin onChange={setFormData} commonData={commonData} resetSignal={resetFormSignal} />;
        } else if (value === 'teacher') {
            return <TeacherJoin onChange={setFormData} commonData={commonData} resetSignal={resetFormSignal}/>;
        } else if (value === 'professor') {
            return <ProfessorJoin onChange={setFormData} commonData={commonData} resetSignal={resetFormSignal}/>;
        }
        return null;
    }
