...
const checkAttendanceStatus = async (studentId) => {
  try {
    const response = await axiosInstance.get(`/attendance/status/${studentId}`);
    if (response.data.status === 'PENDING') {
      setIsPending(true);  // 출석 요청이 대기 중인 경우
    }
  } catch (err) {
    console.error("출석 상태 확인 오류", err);
  }
};
...
const handleAttendanceRequest = async () => {
  ...
  // 출석 상태 확인
  await checkAttendanceStatus(studentId);

  if (isPending) {
    alert("이미 출석 요청이 대기 중입니다.");
    return;  // 출석 요청이 대기 중이면 더 이상 요청을 보내지 않음
  }
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      setIsPending(true);  // 출석 요청 후 대기 상태로 설정
    }
  } catch (err) {
      ...
      <>
        {isApproval ? (
          <>
            ...
            {isMatched && <button onClick={handleAttendanceRequest} disabled={isPending}>출석</button>} {/* 매칭된 경우 출석 버튼 추가 */}
          </>
        ) : (
