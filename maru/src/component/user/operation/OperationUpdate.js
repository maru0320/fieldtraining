...
const OperationUpdate = () => {
  ...
  const fetchBoardDetail = useCallback(async () => {
    try {
      const response = await axiosInstance.get(`/operation/detail?boardID=${boardID}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("/operation/update", formData, {
        headers: {
  ...
  const handleFileDelete = async (fileID) => {
    try {
      await axiosInstance.get(`/operation/update/fileDelete?fileID=${fileID}`);
      alert("파일이 삭제되었습니다.");
